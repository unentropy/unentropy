...
- **Track-Metrics Action**: Composite GitHub Action that orchestrates the four-phase metrics workflow (initialize storage, collect, persist, report)
- **Workflow Phase**: A distinct step in the action (storage initialization, metric collection, persistence, report generation), each with status, duration, and error state
- **Storage Type**: Enumeration of supported storage backends (`sqlite-local`, `sqlite-artifact`, `sqlite-s3`) that determines which storage provider the action uses
- **Action Parameters**: GitHub Action input values including storage type, S3 credentials, artifact configuration, and runtime options
- **Action Outputs**: Set of values emitted by the action after completion including success status, storage type used, database metadata, and workflow duration
