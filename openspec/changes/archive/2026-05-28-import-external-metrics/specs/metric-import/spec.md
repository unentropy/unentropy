...
Defines the behavior of unentropy's metric import pipeline: how external historical metric data enters a local SQLite database via a documented canonical JSONL format. The capability covers only the ingestion side — validation, commit-SHA resolution, and dry-run reporting. Producing the JSONL (from SonarQube or any other source) and moving the resulting database into remote storage are out of scope and handled by external tooling.

## ADDED Requirements

### Requirement: Canonical Import Format Ingestion

As a user with historical metric data prepared in canonical JSONL form, I want a single command that reads the file and writes the corresponding rows into a SQLite database, so that my history is available to `unentropy preview` and to subsequent CI runs once the database is in place.
...
As a user importing historical data from a source that may or may not record git revisions, I want imported measurements to land on real commits when possible, so that trend tooltips show meaningful SHAs and idempotency works against re-imports.
...
As a user (or an agent acting on the user's behalf) about to ingest a file produced by an external dumper, I want to validate the file end-to-end without touching the database, so that I can catch problems before committing to a real write.
...
As a user iterating on an import, I want the command to operate purely against local files, so that re-running, deleting, and adjusting are cheap and reversible — and so that moving the result to remote storage stays the responsibility of agent skills.
...
As a user who has just imported historical data, I want it to appear in my trend charts the next time I generate a report, so that the import actually achieves its goal of seeding history.
