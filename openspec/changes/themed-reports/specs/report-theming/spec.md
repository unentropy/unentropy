...
The report theming system provides CSS-variable-driven color palettes for Unentropy HTML reports. It ships four built-in palettes (Lattice, Flux, Halftone, Specimen) with dark and light variants each, plus the ability for users to supply custom palette overrides. All theming is static — the generator resolves the palette at build time and inlines the variable definitions into the report HTML.
...
- **Palette**: A named collection of 12 CSS color variables for a single theme variant (dark or light).
- **Theme**: A resolved palette pair (dark + light) identified by a name such as "lattice" or "custom".
- **Theme Mode**: The runtime color-scheme strategy — "auto", "light", or "dark".
- **CSS Variable**: A custom property on `:root` or `[data-theme]` that components reference for colors, never hard-coding values.

## Related

- `reporting` — The theming system provides the color layer consumed by the report generator and all report components.
